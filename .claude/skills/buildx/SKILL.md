...
**O planejamento durável é da sprintx.** O briefing declara `max_reprovacoes_f5: 3`, `orcamento_declarado_por: buildx` e `max_replanejamentos_f6: 1`; a F1 os grava no `00-PLANEJAMENTO.md` (`criar <slug> 3 buildx 1`), e dali em diante a sprintx conta as reprovações e a única rodada de replanejamento da execução, e faz os checkpoints locais na `feature/<slug>`. Rodada ativa (`replanejar_execucao`) é feature em execução; os três terminais da F6 — `replanejamento_execucao_esgotado`, `replanejamento_execucao_recusado` (com o motivo em `recusa_replanejamento_f6`) e `orcamento_esgotado` dentro da rodada — são decisão humana, lidos do `00-PLANEJAMENTO.md` commitado, sem worktree, e nunca geram sucessora (D-37, D-38). O buildx decide só pela resposta de `planejamento.sh fase`: `CHECKPOINT` pendente não decide nada — e não vence um `ENTREGA.md` terminal commitado, que decide antes dele (D-35) —, e `persistencia_falhou` para a feature sem virar bloqueio. Commit à frente de `BASE_SHA` antes da F6 pode ser checkpoint legítimo — não é E1, não é entrega.

**Bloqueio nunca para o laço — mas só bloqueia com evidência commitada.** Quando a tentativa termina sem integrar (orçamento da F5 esgotado, replanejamento da execução esgotado, portão bloqueado, entrega interrompida, incompatibilidade), o buildx prova pelo Git que o terminal está no `HEAD` da feature; só então marca `bloqueada` no `MAPA.md` e registra a pendência `aguardando_classificacao` no `RECURSAO.md`, no mesmo commit, e o laço segue. A feature bloqueada nunca volta: é o B5 que decide o que fazer com a pendência dela.
