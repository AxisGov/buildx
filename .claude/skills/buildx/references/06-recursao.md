...
| `commit_nao_registrado` | V11 | — | `trabalho_novo` | a cadeia de evidência do E1 está incompleta — commit de produto sem registro em `ENTREGA.commits` —, e a mergex tem a recuperação mecânica e idempotente (`fechamento-do-e1.sh --registrar-existente`). A sucessora **repara o registro do commit que já existe**; não repete a implementação (D-41) | `entrega_bloqueada/causa_commit_nao_registrado` |
...
- **Converteu** se ao menos uma delas está `entregue` **e** a pendência dela está `resolvida`. `entregue` sem a pendência `resolvida` é o commit de estado incompleto: pare e relate. Sucessora que bloqueou de novo — pelo mesmo ramo ou não — não é conversão. Pendência que mudou para `decisao_humana`, `recurso_externo` ou qualquer outra condição de bloqueio ou interrupção também não é, mesmo quando a mudança vem de evidência nova: a recursão existe para converter trabalho em entrega, e reclassificar não entrega nada (D-34).
