...
| `PARAR` | `orcamento_esgotado` | `duravel` | **portão terminal pré-F6** (`references/05-construcao.md`) — **salvo** com `replanejamento_execucao=ativo` na mesma saída: aí a F5 esgotou o orçamento **dentro** da rodada da F6, e o caminho é o portão terminal da F6 (D-38) |
| `PARAR` | `replanejamento_execucao_esgotado` | `duravel` | **portão terminal da F6** (`references/05-construcao.md`) |
| `PARAR` | `replanejamento_execucao_recusado` | `duravel` | **portão terminal da F6**: a recusa operacional do retorno, com o motivo em `recusa_replanejamento_f6` (D-38) |
...
- **`F6` / `aprovado` com `B-NN` `defeito_de_plano` aberto**: a F6 não retoma a execução normal. O único passo é o `replanejar-execucao` da própria F6 — e a sprintx **decide e grava**: rodada nova, `replanejamento_execucao_esgotado`, ou `replanejamento_execucao_recusado` com o motivo, tudo em checkpoint (DS-148). Nenhuma task nova, nenhuma task concluída tocada, nenhum plano inventado, nenhum orçamento consumido de novo. O buildx não classifica nada daqui: ele lê o terminal do commit, depois. A leitura do `00-BLOQUEIOS.md` do worktree serve só para escolher esse passo enquanto a F6 está viva — e só restringe: nunca move a `CONTROL`.
- **O trabalho parcial da task bloqueada** (sprintx DS-156; D-40): o `replanejar-execucao` não exige mais worktree sem produto sujo. O que a task bloqueada já escreveu — o teste dela, escrito antes do bloqueio, TDD primeiro — e que o plano declara só nela, sem stage, sem segredo, a sprintx **preserva**: grava `path`, `task`, `estado` e `hash` em `parciais_replanejamento_f6` no checkpoint que abre a rodada, e o arquivo continua sujo na árvore até o E1 da task reaberta. A fronteira é dela, inteira: o buildx **não** pré-julga produto sujo antes do passo 3 da F6, e também não lê a chave para decidir nada — quem diz se o parcial continua íntegro é a própria sprintx, em `fase`, `avanca` e no fechamento da rodada. Parcial `perdido` ou `divergente` (`fase` responde `PARAR`, código `2`) é **pare e relate**: o buildx não reconstrói, não restaura, não stasha, não reseta e não transforma o parcial em commit intermediário.
...
- **`replanejamento_execucao_recusado`** (D-38): a recusa **operacional**, gravada e persistida pela sprintx com `recusa_replanejamento_f6: <motivo>`. O buildx a consome do `00-PLANEJAMENTO.md` **commitado** no `HEAD` da feature, lido pela sprintx sobre aquele commit — sem worktree, sem rastro, sem sessão, e sem reexecutar `replanejar-execucao`. Portão terminal da F6, gatilho `replanejamento_execucao_recusado`, `causa: <motivo>`, `decisao_humana`, nenhuma sucessora. Motivo fora do enum da sprintx: **pare e relate**.
- **`orcamento_esgotado` dentro da rodada** (D-38): a F5 consumiu o orçamento restante enquanto o plano voltava à revisão. `fase` responde `PARAR` / `orcamento_esgotado` com `replanejamento_execucao=ativo`, e `bloqueios_replanejamento_f6` preenchido no mesmo commit é a evidência durável de que a rodada existia. Terminal da F6, gatilho `orcamento_f5_esgotado_durante_replanejamento_execucao`, `decisao_humana`, **nenhuma sucessora** — a regra normal criaria uma feature nova a partir da `CONTROL` e perderia as tasks concluídas e os commits válidos daquela branch. Sem rodada aberta, `orcamento_esgotado` continua sendo o terminal pré-F6 de sempre.

**Os motivos da recusa.** `replanejar-execucao` responde `replanejamento=recusado` com um `motivo=` — estes, e só estes, no script fixado (`SPRINTX_SHA_FIXO`, D-42). O buildx pergunta só quando há `defeito_de_plano` aberto, e os lê em duas famílias. A diferença entre elas é o que a sprintx **grava**: a operacional vira o estado terminal `replanejamento_execucao_recusado`, com o motivo em `recusa_replanejamento_f6` e um checkpoint (DS-148) — é durável, sobrevive ao worktree e à sessão, e é dali que o buildx a lê; a de contrato não grava nada, em estado nenhum:
...
| `fronteira_insegura` | `2` | **contrato** | **pare e relate**: há produto sujo que a sprintx não preserva como trabalho parcial da task bloqueada (DS-156; `nao_preservaveis=` diz qual e por quê). Nada é limpo, stashado nem descartado |

Repetir a chamada depois de uma recusa operacional devolve o **mesmo** terminal e o **mesmo** motivo, sem gravar, commitar ou registrar nada de novo (DS-148): o motivo nunca é reavaliado, e o buildx não precisa repetir a chamada para saber dele. `replanejamento=esgotado` não é recusa: é o terminal acima. Os códigos `3` (checkpoint ou fechamento pendente) e `4` (contrato inválido: planejamento malformado, task do `B-NN` que não está `bloqueada`, task concluída alterada durante a rodada) também não são recusa: `3` é a linha **D**, `4` é **pare e relate**. A família **contrato** nunca vira pendência: nada é gravado na `CONTROL`, e o erro fica com quem o produziu.
