...
Avalie **K primeiro**, depois A, e só então as demais.
...
| **D** | `SPRINTX`: `fase=CHECKPOINT`, `persistencia=pendente` | depois de uma transição gravada e não persistida | **complete o checkpoint antes de qualquer decisão do buildx**: peça à sprintx `planejamento.sh checkpoint <slug>`. Não bloqueie, não mexa no mapa, não rode B5, não comece outra feature. Recusado de novo (`persistencia_falhou`): **pare e relate**, preservando worktree e branch |
| **E** | `SPRINTX`: `F3` (`aguardando_f3` ou `replanejar`), `F4` ou `F5`, `persistencia=duravel` | no laço F3 ↔ F5 | continue a sprintx **na fase devolvida**, no mesmo worktree. Não conte reprovação, não regere plano por fora |
| **F** | `SPRINTX`: `F6`, `estado=aprovado`, `persistencia=duravel` — com ou sem commits E1 à frente | aprovado, antes ou durante a execução | siga a F6; a sprintx e a mergex retomam de onde pararam |
| **G** | `SPRINTX`: `PARAR`, `estado=orcamento_esgotado`, `persistencia=duravel`, mapa `em_andamento` | depois do checkpoint terminal, antes do registro do bloqueio | **portão terminal pré-F6** (`references/05-construcao.md`); passou, siga a triagem terminal — só ela move a `CONTROL`. Falhou, **pare e relate** |
...
| **L** | `ENTREGA.md` **commitado** na branch com `estado: entregue` e `portao: pronto`, e **não** `INTEGRADA` | depois do E8, antes do fast-forward | valide o estado commitado (`git show feature/<slug>:…`), a publicação da feature (`push_feito` e `origin/feature/<slug>`) e as seis provas; então integre. **Não reexecute portão, PR ou QA** |
...
| **Q** | branch da feature não descende de `BASE_SHA`; ou à frente, antes da F6, com commit que **não** é checkpoint ou com path fora da pasta da feature; ou `SPRINTX` fora de todas as linhas acima | base envelhecida, trabalho estranho, contrato quebrado | **PARE E RELATE.** O caminho nunca é recriar, rebasear ou integrar à força |
