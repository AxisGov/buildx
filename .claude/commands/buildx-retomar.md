...
| **F** | `SPRINTX`: `F6`, `estado=aprovado`, `persistencia=duravel` — com ou sem commits E1 à frente, e **sem** `ENTREGA` terminal — e **sem** `B-NN` `defeito_de_plano` aberto no worktree | aprovado, antes ou durante a execução — inclusive depois de uma rodada aprovada | siga a F6; a sprintx e a mergex retomam de onde pararam, pelas tasks que restam. **Com** `defeito_de_plano` aberto, a execução normal não retoma (D-37): se a sprintx ainda abriria a rodada ou já gastou o orçamento, a F6 só faz o `replanejar-execucao` do passo 3 dela, e a sprintx decide; se recusou (`classes_mistas`, `orcamento_f6_legado`, `orcamento_f6_nao_declarado`, `planejamento_legado`), a F6 só faz o **fechamento** que materializa a entrega bloqueada — nenhuma task nova. Produto sujo no worktree, ou qualquer outra resposta: **pare e relate** |
...
| **T** | `SPRINTX`: `PARAR`, `estado=replanejamento_execucao_esgotado`, `persistencia=duravel`, **sem** `ENTREGA` terminal, mapa `em_andamento` | depois do checkpoint terminal da F6, antes do registro do bloqueio | **portão terminal da F6** (`references/05-construcao.md`); passou, um único commit de estado: pendência `aguardando_classificacao` no `RECURSAO.md`, gatilho `replanejamento_execucao_esgotado`, com a evidência commitada e os `PR-NN` reservados, feature `bloqueada` no `MAPA.md`, `features_bloqueadas` no `PROJETO.md`. **Nenhuma** rodada nova, **nenhuma** sucessora, **nenhuma** task reaberta. Falhou, **pare e relate** — nada é escrito |
