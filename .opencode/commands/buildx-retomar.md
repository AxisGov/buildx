...
Avalie **K primeiro**, depois A; com a branch existindo, J e a ancestralidade; então **a `ENTREGA` terminal (L, R)** — e só então as demais. A `ENTREGA` terminal vem **antes** de qualquer linha que dependa da `SPRINTX` ou do worktree (D, E, F, G, H, I): o `planejamento.sh` descreve a máquina da sprintx, que termina em `aprovado` e ali fica; a `ENTREGA` terminal descreve o resultado da execução. Uma entrega terminal commitada **não é apagada** por um planejamento que permaneceu `F6` / `aprovado` — nem com o worktree perdido (D-35).
...
| **F** | `SPRINTX`: `F6`, `estado=aprovado`, `persistencia=duravel` — com ou sem commits E1 à frente, e **sem** `ENTREGA` terminal | aprovado, antes ou durante a execução | siga a F6; a sprintx e a mergex retomam de onde pararam |
...
| **Q** | branch da feature não descende de `BASE_SHA`; ou à frente, antes da F6, com commit que **não** é checkpoint ou com path fora da pasta da feature; ou `SPRINTX` fora de todas as linhas acima; ou `ENTREGA.md` commitado que não é ausente, `aberto` nem uma das duas combinações terminais | base envelhecida, trabalho estranho, contrato quebrado | **PARE E RELATE.** O caminho nunca é recriar, rebasear ou integrar à força |
